...
/**
 * Adapter for displaying a grid of {@link ProductItem}s.
 */
...
public class DataAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>
	...
	//Constructor
	public DataAdapter(Activity hostActivity,
	...
	private ProductViewHolder createProductHolder(ViewGroup parent) {
		...
		PhoenixClient prefs = new PhoenixClient(host);
		...
		holder.add.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View view) {
				Map<String, Object> hashMap = new HashMap<>(0); //Create empty map
				Product product = (Product) items.get(holder.getAdapterPosition()); //init product
				
				//Populate map with data
				hashMap.put("id", product.getId());
				hashMap.put("name", product.getName());
				hashMap.put("image", product.getUrl());
				hashMap.put("price", product.getPrice());
				hashMap.put("quantity", product.getQuantity());
				
				//Push to database
				if (prefs.isLoggedIn() && prefs.getCustomer().getKey() != null) {
					if (prefs.isConnected()) {
						prefs.getDb().document("phoenix/orders")   //ref: phoenix/orders
								.collection(prefs.getCustomer().getKey())
								.document()
								.set(hashMap)
								.addOnFailureListener(e -> Toast.makeText(host, e.getLocalizedMessage(),
								...
								.addOnCompleteListener(new OnCompleteListener<Void>() {
									...
									public void onComplete(@NonNull Task<Void> task) {
										if (task.isSuccessful()) {
											Toast.makeText(host,
													String.format("%s has been added to your shopping cart",
											...
											Toast.makeText(host, task.getException().getLocalizedMessage(),
													Toast.LENGTH_SHORT).show();
											holder.add.setText(host.getString(R.string.add_to_cart));
											holder.add.setEnabled(true);
										}
									}
								});
					} else {
						Toast.makeText(host, "Check your internet connection", Toast.LENGTH_SHORT).show();
					}
				}
			}
		});
		
		holder.share.setOnClickListener(view -> new ShareProductTask(host,
				(Product) items.get(holder.getAdapterPosition())).execute());
		return holder;
	}
...
//				.apply(RequestOptions.overrideOf(Target.SIZE_ORIGINAL, Target.SIZE_ORIGINAL))
		...
		if (prefs.isLoggedIn()) {
			prefs.getDb().document(PhoenixUtils.ORDER_REF)
					.collection(prefs.getCustomer().getKey())
					.get()
					.addOnCompleteListener(host, new OnCompleteListener<QuerySnapshot>() {
						@Override
	...
	void addAndResort(List<? extends ProductItem> newItems) {
		Log.d(TAG, "addAndResort: " + newItems);
		weighItems(newItems);
	...
	/*package*/static class ProductViewHolder extends RecyclerView.ViewHolder {
		...
		ProductViewHolder(View itemView) {
			...
			add = itemView.findViewById(R.id.add_product_to_cart);
			share = itemView.findViewById(R.id.share_product);
		}
	}
