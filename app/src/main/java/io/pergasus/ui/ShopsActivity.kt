...
class ShopsActivity : Activity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		if (intent.hasExtra(EXTRA_SHOP)) {
			val shop = intent.getParcelableExtra<Shop>(EXTRA_SHOP)
			name.text = String.format("%s (Loading)", shop.name)
			//Load shop by key, if any else load by name
			if (shop.key.isNullOrEmpty()) loadShop(shop.name)
			else loadShop(shop.key)
		}
