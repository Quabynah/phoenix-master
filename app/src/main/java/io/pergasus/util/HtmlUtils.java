...
public class HtmlUtils {
	...
	private static SpannableStringBuilder linkifyPlainLinks(
			...
			ColorStateList linkTextColor,
			@ColorInt int linkHighlightColor) {
		SpannableString plainLinks = new SpannableString(input); // copy of input
		
		// Linkify doesn't seem to work as expected on M+
		Linkify.addLinks(plainLinks, Linkify.WEB_URLS);
		
		URLSpan[] urlSpans = plainLinks.getSpans(0, plainLinks.length(), URLSpan.class);
	...
	private static SpannableStringBuilder fromHtml(String input) {
		return (SpannableStringBuilder) (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N ? Html.fromHtml(input, Html.FROM_HTML_MODE_LEGACY) : Html.fromHtml(input));
	}
